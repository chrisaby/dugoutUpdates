...
export function PlayoffsView({ locked, semi1, semi2, final: finalMatch }: Props) {
  return (
    <div className="flex flex-col lg:flex-row items-stretch gap-0 max-w-3xl mx-auto">
      ...
      <div className="flex flex-col justify-around gap-6 lg:w-64">
        <PlayoffsSlot label="Semi-final 1" match={locked ? semi1 : null} />
        <PlayoffsSlot label="Semi-final 2" match={locked ? semi2 : null} />
      </div>
      ...
      <div className="flex flex-col justify-center lg:w-64 mt-4 lg:mt-0">
        <PlayoffsSlot label="Final" match={locked ? finalMatch : null} isFinal />
      </div>
    </div>
  )
}

function PlayoffsSlot({
  label,
  match,
  isFinal = false,
}: {
